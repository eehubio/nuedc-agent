...
check("创建项目", p.status === 201 && !!p.data.project_id, JSON.stringify(p.data));
...
check("相同内容不产生新版本(哈希去重)", s2.data.version === s1.data.version);

// 4. 快照：创建 → 改方案 → 整套恢复
const snap = await call("A", `/api/projects/${pid}/snapshots`, { method: "POST", body: "{}" });
check("创建快照", snap.status === 201 && !!snap.data.snapshot_id);
...
check("快照整套恢复", rest.status === 201 && rest.data.restored?.solution >= 3);
...
check("同项目同 Agent 活动任务去重", t2.data.deduped === true && t2.data.task_id === t1.data.task_id);
const c = await call("A", `/api/agent-tasks/${t1.data.task_id}/cancel`, { method: "POST" });
check("排队任务可取消", c.data.status === "canceled");
